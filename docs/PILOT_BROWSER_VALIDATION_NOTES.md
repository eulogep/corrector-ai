...
Après connexion de l’utilisateur au compte Google destiné au projet, une clé API déjà associée au projet `corrector ai` a été sélectionnée dans Google AI Studio et copiée via l’interface sans jamais être affichée, journalisée ou versionnée. Le remplacement de `GEMINI_API_KEY` dans Render est en cours ; il sera suivi d’un redéploiement et d’un test OCR réel.

La page Render est ouverte en édition afin de remplacer `GEMINI_API_KEY` par la clé du projet corrector-ai déjà transférée via le presse-papiers du navigateur. Cette valeur reste masquée et ne sera ni affichée dans les journaux, ni copiée dans un fichier local, ni ajoutée au dépôt.

Les champs de secrets Render ont été refermés immédiatement après la vérification de configuration. Les valeurs ne sont pas reprises dans ce journal. Une rotation préventive des secrets affichés pendant le diagnostic sera proposée avant la clôture du pilote, afin d’éliminer toute exposition résiduelle.

### Diagnostic Gemini et source officielle

Le stockage Supabase est désormais traversé : la réponse OCR contrôlée atteint le fournisseur `gemini`, qui renvoie `ai_provider_unavailable`. L’ancien code utilisait `gemini-1.5-pro`; il a été remplacé par un modèle stable configurable (`gemini-2.5-flash`) dans le commit `41a578e`, avec 40 tests locaux réussis. La documentation Gemini liste `gemini-2.5-flash` comme modèle stable et la documentation de dépréciation indique qu’aucune date de retrait n’est annoncée pour ce modèle à la date de vérification. Les guides officiels confirment le support de l’image en entrée pour les modèles Gemini multimodaux. Sources : https://ai.google.dev/gemini-api/docs/models ; https://ai.google.dev/gemini-api/docs/deprecations ; https://ai.google.dev/gemini-api/docs/image-understanding.

Après connexion au compte Google AI Studio approprié, une clé déjà associée au projet corrector-ai a été vérifiée comme correspondant à la variable Render ; l’acceptation des conditions Gemini a été effectuée avec autorisation explicite. L’erreur OCR persiste donc après mise à jour du modèle, ce qui nécessite de diagnostiquer l’accès effectif à l’API (quota, facturation, restriction de clé ou compatibilité SDK) au moyen d’erreurs fournisseurs catégorisées et sans journaliser de contenu sensible.
