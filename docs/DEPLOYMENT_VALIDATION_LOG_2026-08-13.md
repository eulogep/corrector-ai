...
## Contrôle final après restauration

Le 14 août 2026, le déploiement manuel de la branche `main` au commit `f623ec8` a réussi et Render l’a marqué **Live**. Le point de santé public a répondu `{"status":"ok"}`. Le diagnostic OCR assaini sur la copie synthétique a confirmé une réponse HTTP `503` avec le code `ai_provider_unavailable`, le fournisseur `ocr` et le message générique « Aucun fournisseur OCR n'a pu fournir une extraction exploitable. » Cette réponse est attendue au regard du refus persistant des fournisseurs distants dans cet environnement et respecte le contrat de sûreté : aucune extraction fictive, note ou rapport n’a été produit.
