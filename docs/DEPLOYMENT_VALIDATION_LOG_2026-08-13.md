...
## Repli OCR local de dernier recours

Le diagnostic de la chaîne Gemini–Claude confirme que les deux fournisseurs distants ne fournissent actuellement aucune extraction exploitable depuis Render. Un dernier repli réel, **désactivé par défaut**, a donc été ajouté avec EasyOCR. Il s’exécute localement dans le conteneur, ne requiert ni clé ni appel réseau, conserve uniquement le texte effectivement détecté et dérive le niveau de lisibilité de la confiance retournée par le moteur. Pour le mode structuré, ce texte est soumis au même contrat Pydantic que les fournisseurs distants ; aucune note ni réponse de copie n’est inventée.

La copie synthétique de pilote a été évaluée localement par EasyOCR avec succès : le moteur a détecté 15 segments non vides, pour 281 caractères de texte. Le repli est contrôlé par `LOCAL_OCR_FALLBACK_ENABLED=false` par défaut afin d’exiger une activation explicite et une validation de mémoire/latence sur Render. Les tests backend complets passent à `50 passed` avant publication.
